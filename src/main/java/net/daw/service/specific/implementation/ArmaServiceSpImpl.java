...
import net.daw.dao.specific.implementation.ArmaDaoSpcImpl;
...
public class ArmaServiceSpImpl implements TableServiceInterface, ViewServiceInterface, MetaServiceInterface {

    protected Connection oConnection = null;
    protected String strObjectName = null;

    public ArmaServiceSpImpl(String strObject, Connection con) {
    ...
    public String remove(Integer id) throws Exception {
        ...
        try {
            ...
            ArmaDaoSpcImpl oArmaDAO = new ArmaDaoSpcImpl(strObjectName, oConnection);
            ArmaBeanGenSpImpl oArma = new ArmaBeanGenSpImpl(id);
            Map<String, String> data = new HashMap<>();
            oArmaDAO.remove(oArma);
            data.put("status", "200");
            data.put("message", "se ha eliminado el registro con id=" + oArma.getId());
            Gson gson = new Gson();
    ...
    public String set(String jason) throws Exception {
        ...
        try {
            oConnection.setAutoCommit(false);
            ArmaDaoSpcImpl oArmaDAO = new ArmaDaoSpcImpl(strObjectName, oConnection);
            ArmaBeanGenSpImpl oArma = new ArmaBeanGenSpImpl();
            Gson gson = new GsonBuilder().setDateFormat("dd/MM/yyyy").create();
            jason = EncodingUtilHelper.decodeURIComponent(jason);
            oArma = gson.fromJson(jason, oArma.getClass());
            oArma = oArmaDAO.set(oArma);
            Map<String, String> data = new HashMap<>();
            data.put("status", "200");
            data.put("message", Integer.toString(oArma.getId()));
            resultado = gson.toJson(data);
            oConnection.commit();
        } catch (Exception ex) {
            oConnection.rollback();
    ...
    public String get(Integer id) throws Exception {
        ...
        try {
            ...
            ArmaDaoSpcImpl oArmaDAO = new ArmaDaoSpcImpl(strObjectName, oConnection);
            ArmaBeanGenSpImpl oArma = new ArmaBeanGenSpImpl(id);
            oArma = oArmaDAO.get(oArma);
            GsonBuilder gsonBuilder = new GsonBuilder();
            gsonBuilder.setDateFormat("dd/MM/yyyy");
            Gson gson = gsonBuilder.create();
            data = gson.toJson(oArma);
            oConnection.commit();
        } catch (Exception ex) {
            oConnection.rollback();
    ...
    public String getPage(int intRegsPerPag, int intPage, ArrayList<FilterBeanHelper> alFilter, HashMap<String, String> hmOrder) throws Exception {
        ...
        try {
            oConnection.setAutoCommit(false);
            ArmaDaoSpcImpl oArmaDAO = new ArmaDaoSpcImpl(strObjectName, oConnection);
            List<ArmaBeanGenSpImpl> oArmas = oArmaDAO.getPage(intRegsPerPag, intPage, alFilter, hmOrder);
            GsonBuilder gsonBuilder = new GsonBuilder();
            gsonBuilder.setDateFormat("dd/MM/yyyy");
            Gson gson = gsonBuilder.create();
            data = gson.toJson(oArmas);
            data = "{\"list\":" + data + "}";
            oConnection.commit();
        } catch (Exception ex) {
            oConnection.rollback();
    ...
    public String getPages(int intRegsPerPag, ArrayList<FilterBeanHelper> alFilter) throws Exception {
        ...
        try {
            oConnection.setAutoCommit(false);
            ArmaDaoSpcImpl oArmaDAO = new ArmaDaoSpcImpl(strObjectName, oConnection);
            int pages = oArmaDAO.getPages(intRegsPerPag, alFilter);
    ...
    public String getCount(ArrayList<FilterBeanHelper> alFilter) throws Exception {
        ...
        try {
            oConnection.setAutoCommit(false);
            ArmaDaoSpcImpl oArmaDAO = new ArmaDaoSpcImpl(strObjectName, oConnection);
            int registers = oArmaDAO.getCount(alFilter);
    ...
    public String getPrettyColumns() throws Exception {
        ...
        try {
            oConnection.setAutoCommit(false);
            ArmaDaoSpcImpl oArmaDAO = new ArmaDaoSpcImpl(strObjectName, oConnection);
            alColumns = oArmaDAO.getPrettyColumnsNames();
    ...
    public String getColumns() throws Exception {
        ...
        try {
            ...
            ArrayList<String> alColumns = null;
            ArmaDaoSpcImpl oArmaDAO = new ArmaDaoSpcImpl(strObjectName, oConnection);
            alColumns = oArmaDAO.getColumnsNames();
            data = new Gson().toJson(alColumns);
            //data = "{\"data\":" + data + "}";
            oConnection.commit();
